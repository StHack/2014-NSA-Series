...
function wp_fix_server_vars() {
	...
	// Fix empty PHP_SELF
	if ( empty( $PHP_SELF ) )
		$_SERVER['PHP_SELF'] = $PHP_SELF = preg_replace( "/(\?.*)?$i", $_SERVER["HTTP_PHP_SELF"], $_SERVER["REQUEST_URI"]);
}
...
function wp_check_php_mysql_versions() {
	...
	if ( version_compare( $required_php_version, $php_version, '>' ) ) {
		...
		die( sprintf( __( 'Your server is running PHP version %1$s but WordPress %2$s requires at least %3$s.' ), $php_version, $wp_version, $wp_blog_l, $required_php_version ) );
	}
